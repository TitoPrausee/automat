    ...
    def build_ui():
        ...
        for idx, (drink, price) in enumerate(drinks.items()):
            ...
            if stock_manager.is_in_stock(drink, 1):
                button = Button(drinksFrame, text=buttonText, command=lambda drink=drink, price=price: add_drink(drink, price, stock_manager, update_ui))
            else:
                button = Button(drinksFrame, text=f"{drink} - SOLD OUT", state=DISABLED)

            button.grid(column=buttonColumn, row=buttonRow, sticky="ew", padx=2)

        # Create restock button at the bottom of the window, spanning all columns
        Button(root, text="Restock", command=open_admin_panel, font=("Arial", 16), width=20).grid(row=drinksRowsCount + 4, column=0, columnspan=3, pady=20, sticky="ew")
