...
from Backend.Tools.transaction_manager import TransactionManager
from Backend.DataAccessors.Stock import Stock
# Import AdminPanelgernerate_drink_buttons
from UI.admin_panel import AdminPanel

# TODO: fetch drinks from database & filter drinks that are out of stock
drinks = [
    {"name": "Cola", "price": 1},
    {"name": "Fanta", "price": 2},
    {"name": "Sprite", "price": 3.5},
    {"name": "Wasser", "price": 5.1},
]
...
def create_UI(root, price_calculator):
    ...
    # Column 1 (centered labels will be here)
    ...
    guthabenLabel = Label(root, text="Guthaben: " +
                          guthaben.__str__(), font=("Arial", 20))
    guthabenLabel.grid(row=1, column=1, pady=30, sticky="NESW")

    moneyFrame = tk.Frame(root, bg="white", borderwidth=2,
                          relief="sunken", height=250)
    ...
    for i, coin in enumerate(coin_values):
        Button(moneyFrame, highlightbackground="white", text=f"{
               coin}€", command=lambda value=coin: enter_money(value)).grid(row=0, column=i, sticky="we")
    ...
    for i, bill in enumerate(bill_values):
        Button(moneyFrame, highlightbackground="white", width=8, text=f"{
               bill}€", command=lambda value=bill: enter_money(value)).grid(row=1, column=i, sticky="we")

    drinksFrame = tk.Frame(root, bg="white", borderwidth=2,
                           relief="sunken", height=250)
    ...
    # TODO: refetch drinks after each transaction to match the stock
    gernerate_drink_buttons(drinks)

    Button(root, text="Ausgabe", state=DISABLED).grid(
        row=4, column=1, sticky="ns", pady=10)
...
def gernerate_drink_buttons(drinks):
    ...
    for i, drink in enumerate(drinks):
        ...
        if currentColumn < 0:
            currentColumn = 0
        Button(drinksFrame, text=f"{drink["name"]} ({drink["price"]} €)", highlightbackground="white", width=7).grid(
            column=currentColumn, row=currentRow, sticky="ew")
...
#             # Get the product quantity from stock.csv
...
#                 button = Button(root, text=buttonText, command=lambda drink=drink,
#                                 price=price: add_drink(drink, price, stock_manager, update_ui))
#             else:
#                 button = Button(
#                     root, text=f"{drink} - SOLD OUT", state=DISABLED)
...
#         coin_values = [0.5, 1, 2]
#         for i, coin in enumerate(coin_values):
#             Button(root, text=f"{coin}€", command=lambda value=coin: enter_sum(
#                 value)).grid(row=drinksRowsCount + 4, column=i)

#         # Buttons for entering bills
#         bill_values = [10, 20]
#         for i, bill in enumerate(bill_values):
#             Button(root, text=f"{bill}€", command=lambda value=bill: enter_sum(
#                 value)).grid(row=drinksRowsCount + 5, column=i)

#         Button(root, text="Finish", command=reset).grid(
#             row=drinksRowsCount + 6, column=0)

#         # Button for admin access to restock products
#         Button(root, text="Restock", command=open_admin_panel).grid(
#             row=drinksRowsCount + 7, column=0)
...
#         # Simple password check for demonstration (replace with secure method in production)
#         if password == "123":
...
#             # Pass the update_ui callback to dynamically update the UI
...
#     # Use the 'update_stock' method of the 'Stock' class
#     stock_manager.update_stock(drink, 1)

#     update_labels()
#     # Log the transaction (1 item per click)
...
#     labelSumToEnter.config(text=f"Still to enter: {
#                            max(sumToEnter - enteredSum, 0)}")
